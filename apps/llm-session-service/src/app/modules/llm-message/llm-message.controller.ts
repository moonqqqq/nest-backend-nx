...
export class LlmMessageController {
  ...
  @MessagePattern('get_llm_messages')
  async getLlmMessages(
    @Payload() { userId, llmSessionId }: GetLlmMessagesPayload,
  ) {
    return await this.llmMessageService.getLlmMessages(userId, llmSessionId);
  }
}
