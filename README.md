...
5. LLM Streaming 백그라운드 파이파라인 구축(API서버와 완전한 분리) (Redis)
  - [ ] 5.1. LLM서버로부터 Stream으로 받고 있는 데이터들을 임시 저장하는 저장소 구현
  - [ ] 5.2. 하나의 질문에 응답하는 동안만 stream 임시 저장소 살아있도록 리소스 효율화
  - [ ] 5.3. 하나의 스트리밍으로 부터 들어오는 이벤트들을 여러 곳에서 구독할수 있는 프로세스 구현 (ex. 사용 토큰 계산, 통계처리 등 파이프라인 시작단계)
  - [ ] 5.4. 에러 처리
...
7. LLM 응답 이벤트를 구독하는 파이프라인
  - [ ] 토큰 사용량 측정 파이프라인(유저 플랜에 따른 제약 구현)
  - [ ] 로그 수집 파이프라인
